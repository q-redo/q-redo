...
const getInfoAndEmit = async socket => {
  ...
  try {
    const userres = await db.run(
      `SELECT * FROM users WHERE logged_in = true AND rank = 3;`
    );
    const mentorres = await db.run(
      `select * FROM users WHERE logged_in = true AND rank = 2`
    );
...
app.put('/api/userAnsweredQuestion/:id', controller.userAnswered);
...
app.get('/api/getTimeAsked', controller.getTimeAsked);
