...
class TopicCharts extends Component {
  ...
  theCallBack() {
    ...
    if (this.state.cohortFilter > 0 && phraseHolder.length < 19) {
      phraseHolder = phraseHolder.concat(
        ` WHERE cohort_id = ${this.state.cohortFilter}`
      )
    }
    if (this.state.campusFilter > 0 && phraseHolder.length < 19) {
      phraseHolder = phraseHolder.concat(
        ` WHERE campus_id = ${this.state.campusFilter}`
      )
    } else if (this.state.campusFilter > 0 && phraseHolder.length >= 19) {
      phraseHolder = phraseHolder.concat(
        ` AND campus_id = ${this.state.campusFilter}`
      )
    }
    if (this.state.answeredFilter.length > 0 && phraseHolder.length < 19) {
      phraseHolder = phraseHolder.concat(
        ` WHERE answered = ${this.state.answeredFilter}`
      )
    } else if (
      this.state.answeredFilter.length > 0 &&
      phraseHolder.length >= 19
    ) {
      phraseHolder = phraseHolder.concat(
        ` AND answered = ${this.state.answeredFilter}`
      )
    }
    if (this.state.topicFilter > 0 && phraseHolder.length < 19) {
      phraseHolder = phraseHolder.concat(
        ` WHERE topic_id = ${this.state.topicFilter}`
      )
    } else if (this.state.topicFilter > 0 && phraseHolder.length >= 19) {
      phraseHolder = phraseHolder.concat(
        ` AND topic_id = ${this.state.topicFilter}`
      )
    }
    ...
    axios
      ...
      .then(() => {
        if (this.state.graphType === "UVA") {
          ...
          this.state.searchContent.forEach(
            occ =>
              occ.answered === false
          ...
          this.state.allTopics.forEach(
            occ =>
              blankGraph.labels.push(occ.topic) &
              blankGraph.datasets[0].data.push(
                this.state.searchContent.filter(id => id.topic_id === occ.id)
                  .length
              ) &
              blankGraph.datasets[0].backgroundColor.push(
                "rgb(".concat(
  ...
  getcohortandcampus() {
    ...
    let cohHolder = []
    let cohObjects = []
    axios.get("/api/getcampusandcohort").then(response => {
      console.log(response)
      ;response.data.map(
        (campi, i) =>
          campHolder.indexOf(campi.campus_id) < 0
            ? campHolder.push(campi.campus_id) &
              campObjects.push(campi) &
              this.setState({ noDupCampus: campObjects })
            : false
      ) &
  ...
  render() {
    ...
    const topicList = allTopics.map((topz, i) => (
      <option value={topz.id} key={i}>
        {topz.topic}
      </option>
    ))
    const campusList = noDupCampus.map((campz, i) => (
      <option value={campz.campus_id} key={i}>
        {campz.campus_name}
      </option>
    ))
    const cohortList = cohorts.map((coh, i) => (
      <option value={coh.cohort_id} key={i}>
        {coh.formal_name} of {coh.campus_name}
      </option>
    ))
    return (
      <div>
        <div className="big-circle-card m10">
          <button
            onClick={() => this.showBox() & this.getcohortandcampus()}
          ...
          <div className="curved shadowed m10 adminpopupbox">
            <div className="admincenterboxcontent">
              <div>
                ...
                >
                  <option value="archived_questions">Archived Questions</option>
                  <option value="questions">Active Questions</option>
                </select>
                <select
                  defaultValue="0"
                  onChange={e => this.handleCampusFilter(e.target.value)}
                >
                ...
                <select
                  defaultValue="0"
                  onChange={e => this.handleCohortFilter(e.target.value)}
                >
                ...
                <select
                  defaultValue=""
                  onChange={e => this.handleAnsweredFilter(e.target.value)}
                >
                ...
                <select
                  defaultValue="0"
                  onChange={e => this.handleTopicFilter(e.target.value)}
                >
